...
lnastep = function(model, z, m, t, ddt)
{
      ...
      sol = deSolve:::ode(y=state, func=issb:::lnafun, times=seq(0, t, by = ddt), parms=model)
...
h = function(x, pars) {
      ...
      hazs[1] = pars[1] * (pars[3] - x[1]) * (pars[4] + x[1] - x[2])   # beta * S * I
      hazs[2] = pars[2] * (pars[4] + x[1] - x[2])        # mu * I
      return(hazs)
}
...
# simulate a path
set.seed(52787)
path = lna(model = model, maxtime = 10, ddt = 0.01, restart = TRUE)
...
yseqs <- 
      rep(path$pathmat[path$pathmat[,1] %%1 == 0,2][-1], each = length(seq(-4,4,by=0.01))) + 
      rep(seq(-4,4,by=0.01), 10) * rep(sqrt(path$diff_mats[1,1,]), each = length(seq(-4,4,by=0.01)))

lna_dists = data.frame(loc_x = rep(seq(1,10), each = length(seq(-4,4,by=0.01))),
                       x = yseqs,
                       dens = dnorm(seq(-4,4,by=0.01)))

lna_path = as.data.frame(path$pathmat)
lna_path$interval <- c(1,rep(1:10,each = 100))
...
ggplot(lna_path, aes(x = Time, y = N_SI, group = as.factor(interval))) + 
      ...
      theme_bw() + 
      labs(x = "Time", 
           y = bquote(paste(bold(N)[SI](bold(theta)))), 
           title = bquote(paste("Centered parameterization: ", bold(N) ~"|"~ bold(theta) %~%" MVN"(bold(mu)(bold(theta)), bold(Sigma)(bold(theta)))))) + 
...
ggplot(gaussians, aes(x = loc_x + dens, y = x, group = loc_x))+
      ...
      labs(x = "Time", 
           ...
           title = bquote(paste("Non-centered parameterization: ", bold(N) %=>% doLNA(bold(Z)~","~theta)))) + 
