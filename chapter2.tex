...
\end{equation}
\vmcomment{It is strange that you are talking about integral in transition densities, but then show the integral in the overall posterior. Not incorrect, but not intuitive.}
...
The waiting times between subject--level transition events are typically taken to be exponentially distributed. This will allow us to take advantage of several useful properties of exponential random variables (\vmdel{Figure} \vmadd{List} \ref{list:exp_props}, see \cite{wilkinson2011stochastic} for proofs). Critically, this choice also implies that $ \bX $ evolves as a continuous--time Markov chain (CTMC) with transition rate from configuration $ \bX $ to $ \bX^\prime $, differing only in the state of a single subject, given by
\begin{equation}
...
\begin{framed}
	\begin{itemize}
		...
		\end{equation} 
	\end{itemize}
	\vmcomment{I would also add that $\text{min}(Z_i)$ and $k$ are independent.}
\end{framed}
...
\vmcomment{You have to be careful here. First, you are conditioning on the rest of the trajectories. Second, are you sure this is the right full conditional? I am not. Lastly, I would remove this paragraph from the intro and discuss this technicality only in chapter 3.}.
\begin{equation} 
...
\vmcomment{Is commutativity here to make things more technically complete? I don't see why we need to talk about it. Are we using heavily later?}
...
where $ \bx,\bx^\prime\in\mcS $ and $ 0\leq\tau\leq\tau^\prime $. The transition kernel only depends on the time--elapsed when the process is time--homogeneous $ P_{\bx,\bx^\prime}(\tau,\tau^\prime) = P_{\bx,\bx^\prime}(|\tau^\prime - \tau|) \equiv P_{\bx,\bx^\prime}(\tau^\prime) $. 
\vmcomment{typo in the second equality; not sure what you wanted to say there}
In matrix form, the transition kernel, $ P $, is an $ r\times r $ stochastic matrix, where $ r = |\mcS| $, the rows of $ P $ sum to one, and $ P_{ii} = -\sum_{j\neq i}P_{ij}$. 
\vmcomment{It is confusing that you lost time dependence of P in the above sentence.}
Trivially, $ P_{\bx,\bx^\prime}(0) = I $, i.e., there are no state changes in a time interval of zero length, almost surely. By the Markov property, the probability of transitioning from state $ i $ to state $ k $ in time $ s+t $ is 
\begin{equation}
\label{eqn:chapman_kolmogorov}
P_{ik}(s+t) = \sum_{j\in\mcS}P_{ij}(s)P_{jk}(t),
\end{equation}
or in matrix form, $$P(s+t) = P(s)P(t).$$
\vmcomment{Don't forget to bold matrices and vectors.}
...
\begin{align*}
\Pr\left (\bX(\tau+\dtau) = (S-1, I+1, R) | \bX(\tau) = (S,I,R)\right ) &= \beta SI {\color{red} \dtau} + o(\dtau), \\
\Pr\left (\bX(\tau+\dtau) = (S, I-1, R+1) | \bX(\tau) = (S,I,R)\right ) &= \mu I {\color{red} \dtau} + o(\dtau).
\end{align*}
...
\vmcomment{I appreciate that you are trying to connect everything here, but your statement is not very precise. For example, I don't think Kolmogorov equations exist for ODE formulations --- you need probability for this. You probably mean asymptotic mean, but it is hard to tell from the sentence.}
...
\subsubsection{Deterministic representation as a system of ODEs \vmcomment{Decide on capitalization in subsection titles and stick with it}}
\label{subsec:deterministic_models}

In the infinite population limit \vmcomment{Why not to be precise here and define $S(t) = \lim_{N \rightarrow \infty} E(S(t))$. This definition is useful, because it shows that you shouldn't be using the same letter $S$ in both stochastic and deterministic equations.}, sample paths of the SIR model are solutions to the following system of ODEs, subject to the initial constraint that $ \bX(\tau_0) = \bx_0 $:
\begin{align*}
...
\begin{enumerate}
	...
	\end{equation}
	\vmcomment{Hmm, where was $\blambda$ notation/meaning defined?}
