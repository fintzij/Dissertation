...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figures/subject_sample_paths}
...
\end{align*}
For example, the infinitesimal transition probabilities for the population--level SIR model are
\begin{align*}
...
\begin{figure}[htbp]
	...
	\draw (0,7) circle(1.25) node (MJP) {MJP};
	\draw (15,7) circle(1.25) node (SDE) {SDE};
	\draw (30,7) circle(1.25) node (ODE) {ODE};
	\draw (15,0) circle(1.25) node (LNA) {LNA};
	
	\draw [thick,shorten >=0.1cm,shorten <=0.1cm,->] (MJP) -- (SDE) node[midway,above] {Normal approximation};
	\draw [thick,shorten >=0.1cm,shorten <=0.1cm,->] (SDE) -- (ODE) node[midway,above] { Limit as $ N \rightarrow \infty$};
	\draw [thick,shorten >=0.1cm,shorten <=0.1cm,->] (LNA) -- (ODE) node[midway,right,yshift=-0.5cm,xshift=-0.5cm] {Mean};
	\draw [thick,shorten >=0.5cm,shorten <=0.5cm,->] (SDE) -- (LNA) node[midway,right,yshift=0.35cm] { Taylor expansion};
	\draw [thick,shorten >=0.1cm,shorten <=0.1cm,->] (MJP) -- (LNA) node[midway,left,yshift=-0.5cm,xshift=0.5cm]{ System size expansion};
	\draw [thick, shorten >= 0.1cm, shorten <=0.1cm, ->] (MJP) to [out=30, in=150] (ODE); \draw (15,12.5) node {Limit as $N\rightarrow\infty$};
	\end{tikzpicture}
	\caption[Relationship between various compartmental model representations.]{Relationship between various compartmental model representations. The Markov jump process (MJP) is typically the model of interest, and in large populations can be approximated by a system of stochastic differential equations (SDE). The linear noise approximation (LNA) is a tractable approximation obtained by Taylor expansion of the SDE about its deterministic limit, given by a system of ordinary differential equations (ODE), or by a system size expansion of the backward Kolmogorov equation of the MJP.}
	\label{fig:mjp_approximations}
\end{figure}
...
where $ \pi(\by|\btheta) $ and $ \pi(\btheta) $ are the sampling distribution and the prior distribution. The marginal distribution of the data, $ \pi(\by) = \int\pi(\by|\btheta)\rmd\btheta $, is constant with respect to $ \btheta $, and so we typically work with the unnormalized posterior, \begin{equation*}
\pi(\btheta|\by)\ \propto\ \pi(\by|\btheta)\pi(\btheta).
\end{equation*} 
...
It can be challenging to design an MCMC scheme that explores the posterior efficiently. The theory of MCMC merely guarantees that samples from a Markov chain will approximate the target posterior if we run the Markov chain long enough. In practice, the choice of MCMC transition kernel, along with the model parameterization, are critically important to the computational efficiency of the MCMC and, moreover, to the fidelity of the MCMC approximation to the target posterior \cite{betancourt2017conceptual}. We now present several MCMC algorithms that will be used throughout this dissertation. 

\subsubsection{Metropolis--Hastings sampler}
\label{subsubsec:metropolis_hastings}

One of the most important MCMC samplers, due to its simplicity and generality, is the Metropolis--Hastings (MH) algorithm \cite{hastings1970monte,metropolis1953equation}. At each step in the Markov chain, a new state is proposed from the distribution $ q(\prop{\btheta}|\cur{\btheta}) $, and is accepted with probability
\begin{align}
\label{eqn:metropolis_hastings_1}
\alpha_{\cur{\btheta}\rightarrow\prop{\btheta}} &= \min\left \lbrace 1, \frac{\pi(\prop{\btheta}|\bY)}{\pi(\cur{\btheta}|\bY)}\frac{q(\cur{\btheta}|\prop{\btheta})}{q(\prop{\btheta}|\cur{\btheta})}\right \rbrace \\
\label{eqn:metropolis_hastings_2}
&= \min\left \lbrace 1, \frac{\pi(\by|\prop{\btheta})\pi(\prop{\btheta})}{\pi(\by|\cur{\btheta})\pi(\cur{\btheta})}\frac{q(\cur{\btheta}|\prop{\btheta})}{q(\prop{\btheta}|\cur{\btheta})}\right \rbrace.
\end{align}
...
We introduce slice sampling by considering the task of sampling a random variable, $ \theta\in\bbR $ with density $ \pi(\theta)\propto f(\theta) $.  Rather than sample $ \theta $ directly, we introduce an auxiliary variable, $ u $, defined such that the joint density of $ (\theta,u) $ is uniform over the area $ \bA = \lbrace (\theta,u):\ 0<u<f(\theta) \rbrace. $ Slice sampling alternates Gibbs updates of $ \theta|u $ and $ u|\theta $ from their full conditionals,
\begin{align*}
