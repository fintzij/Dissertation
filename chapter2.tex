...
Markov chain Monte Carlo (MCMC) is a method for approximating a posterior from which it is not possible to draw samples directly. The strategy is to construct an ergodic (aperiodic and irreducible) Markov chain, $ \lbrace \theta_n\rbrace $, whose stationary distribution is $ \pi(\theta|y) $. The transition kernel of a Markov chain, $ K(x,x^\prime) $, gives the probability of moving from $ x $ to $ x^\prime $, which are elements in the support of the stationary distribution. The Markov chain preserves the stationary distribution if it satisfies the global balance condition,
$$\pi(x) = \int K(x^\prime,x)\pi(x^\prime)\rmd x^\prime.$$
...
$$\pi(x)K(x^\prime,x) = \pi(x^\prime)K(x,x^\prime),\ \forall x,x^\prime.$$
...
One of the most important MCMC constructions, due to its simplicity and generality, is the Metropolis--Hastings (MH) algorithm \cite{metropolis1953equation,hastings1970monte}. At each step in the Markov chain, a new state is proposed from the distribution $ q(\prop{\theta}|\cur{\theta}) $, and is accepted with probability
\begin{align}
...
\begin{algorithm}[htbp]
	...
	\label{alg:gibbs}
	\begin{algorithmic}[1]
		\Procedure{Gibbs}{$ \cur{\theta},\ \mcI $ parameter/block indices.}
		\For{$ J\in\mcI $}
			\State Update: $ \theta_J|\theta_{-J},y\sim\pi(\theta_J|\theta_{-J},y) $
		\EndFor
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
Given the current state $ \theta $, threshold $ u $, and initial bracket width $ \omega $, we can use the ``stepping out" procedure to expand the interval until it covers $ S $. If the resulting interval is too wide, we can subsequently shrink it using the rejected draws by shrinking the appropriate bracket endpoint to the rejected value. We will typically tune the initial bracket width throughout the MCMC run, as in \cite{tibbits2014automated}, to target an equal number of bracket expansions to shrinkages.
...
The elliptical slice sampler (ElliptSS) of \cite{murray2010} is an algorithm for efficiently sampling normally distributed random variables. The conditions we need for ElliptSS to be valid are that $ \bx\sim N(\mathbf{0}, \bSigma) $, and that the posterior factors as $ \pi(\bx|\by)= \frac{1}{Z}N(\bx;\bs{0}, \bSigma)L_\by(\bx) $. The strategy behind ElliptSS is to reparameterize the model by introducing auxiliary variables in a way that preserves the marginal distribution of $ \bx $, thus preserving the likelihood of the data conditional on $ \bx $.

We introduce auxiliary variables, $
\boeta_0 \sim N(\bs{0}, \bSigma),\
\boeta_1 \sim N(\bs{0}, \bSigma),\
\theta \sim \mr{Unif}(0, 2\pi)$, and define $
\bx = \boeta_0\sin(\theta) + \boeta_1\cos(\theta). $ Note that $ \bx\sim N(\bs{0}, \bSigma) $, so we have not changed its marginal distribution. ElliptSS alternates sampling $ \boeta_0,\boeta_1,\theta $ under the constraint that $ \bx $ is unchanged, and updating $ \theta $ by univariate slice sampling. To this end, the model is reparameterized as, $$\theta \sim \mr{Unif}(0, 2\pi),\
\boeta \sim N(\bs{0}, \bSigma),\
\boeta_0 = \bx \sin(\theta) + \boeta \cos(\theta),\
\boeta_1 = \bx\cos(\theta) - \boeta\sin(\theta),$$ which leaves the latent variable unchanged as $ \prop{\bx} = \boeta_0\sin(\theta) + \boeta_1\cos(\theta) = \bx$. In practice, we need not compute $ \boeta_1 $ since it will be discarded anyway.
