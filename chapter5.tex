...
We will use the fit a vaccination--age stratified susceptible--infected--recovered--susceptible (SIRS) model (Figure \ref{fig:flu_sirs_diag}) in which individuals transition stochastically, and continuously in time, between disease states. Individuals become infectious immediately upon becoming infected, and acquire temporary, though potentially long lasting, protection upon recovery. Individuals who lose immunity are assumed to become fully susceptible. For simplicity, we will ignore demographic changes or mortality since the age groupings are considerably coarser than the time period under consideration. 
...
1) exponential sojourn time distributions, specify rates, but not yet timevarying FOI -> MJP, approximate with LNA
...
\begin{table}[htbp]
	...
	\begin{tabular}{lll}
		...
		$ N_j^{(k)} $ & Size of age stratum $ j $ with vaccination status $ k $ & Yes \\
