...
The second diagnostic, motivated by ideas presented in \cite{lau2014new}, leverages the structure of LNA draws as i.i.d. standard normal variates to qualitatively assess the discrepancy between the posterior distribution of the latent process and the LNA prior. The LNA draws are i.i.d. N(0,1) variates, and the quantiles of their left tail probabilities, $ \bU_\bZ \equiv \Pr(\Phi(\bZ) < q) $, are uniformly distributed \textit{a priori}. The empirical quantiles of the posterior left tail probabilities with respect to their standard normal prior, $ \bU_\bZ|\bY \equiv \Pr\left (\Phi(\bZ) < q | \bY\right ) $, can be understood as latent recursive residuals in the sense of \cite{cox1968general} since each LNA increment is a deterministic function of the model parameters and the LNA draws, $ \bZ $. Discrepancies between the latent recursive residual distributions and their uniform priors suggests that the latent process dynamics, conditional on all of the observed data, may be unlike those expected under the LNA prior. Figure \ref{fig:sirresidplots} shows the LNA residuals for an SIR model fit to incidence data from an outbreak with SIR dynamics. Even though there was no model misspecification, we see that quantiles of the latent residuals are clearly not uniform towards the start and end of the outbreak. There are relatively few infection and recover events during these time periods, so one post--hoc explanation is that the LNA poorly approximates the latent process at these times. In contrast, the LNA residuals closely align with expected prior quantiles during the exponential growth and decay of the outbreak, which is exactly when we expect the LNA approximation to be most appropriate. It is important to emphasize that the LNA residuals merely point to discrepancies between the latent posterior and its functional prior, but do not diagnose the source of the discrepancy. The LNA residuals may also be sensitive to influential data points that are unlikely under absent an excess (or dearth) of infections.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figures/sir_residplots}
	\caption[SIR model latent recursive residuals.]{Latent recursive residuals, computed as left tail probabilities of the posterior LNA draws for infections (left) and recoveries (right). Solid lines of varying shades correspond to the latent residuals, light horizontal lines in the background correspond to the theoretical quantiles under the N(0,1) prior.}
...
We now apply our methods to model the spread of Ebola in Guinea, Liberia, and Sierra Leone during the 2013--2016 Ebola outbreak. The data, shown in Figure \ref{fig:eboladat}, consist of national case counts from the World Health Organization patient database consisting of weekly confirmed and probable Ebola cases \cite{who2016eboladat}. Individuals were classified as suspected cases if they presented with Ebola--like symptoms and had contact with a suspected, probable, or confirmed case of a dead or sick animal. Probable cases consisted of suspected cases who were evaluated by a clinician, or who had died but were epidemiologically linked to confirmed cases. A confirmed case was defined as a suspected or probable case testing positive for Ebola virus RNA or IgM Ebola antibodies \cite{coltart2017ebola}. 
...
There are several reasons to believe \textit{a priori} that our dataset significantly under--counts the true incidence, and that reporting rates exhibit substantial variation from country--to--country. First, we have not included suspected cases, many of which were true cases, since they were not reported by the WHO in this dataset. Suspected cases accounted for sizable fractions of the total case counts (see Table \ref{tab:ebola_descriptives}) and may have been particularly important at the beginning and peak of the outbreak when health care systems were under--resourced.
