...
\label{eqn:intractable_posterior}
 \pi(\btheta|\bY)&\propto \pi(\bY|\btheta)\pi(\btheta) = \int L(\bY|\bN^c,\btheta)\pi(\bN^c|\btheta)\pi(\btheta)\rmd\bN^c\nonumber\\
 &= \int_{\mcS^c}\prod_{\ell=1}^L \Pr\left (Y_\ell|\Delta\bN_{SI}^c(t_\ell),\btheta\right )\pi\left (\bN^c(t_\ell)|\bn^c(t_{\ell-1}),\btheta\right )\pi(\btheta)\rmd\bN^c\vmadd{,}
\end{align}
...
\begin{table}[htbp]
	...
	\begin{tabular}{cllc}
		...
		$ R_{\color{red} 0}\vmdel{0}-1 $ & Basic reproduction \# - 1 & LogNormal(0, 0.5) & $ \implies _{\color{red} 0}\vmdel{0} = $ 2.00 (1.38, 3.66) \\ 
