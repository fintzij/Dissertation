...
Before proceeding, we pause to note that the effective population size and case detection probability and effective population size are only weakly identifiable in this model. Rather, the mean case detection rate, $ \rho N_{eff} $, is estimable because of its relationship to the true and observed outbreak sizes. In practice, we require prior information about either $ \rho $ or $ N_{eff} $ in order to be able to estimate the parameters separately. This is is discussed further in Section \ref{sec:effpop_identifiability}. 

We fit the model using a combination of weakly informative priors for the effective reproductive numbers, informative priors for the latent period durations, infectious period durations, and cross--border transmission parameters, and diffuse priors for the measurement process parameters and effective population sizes. The model was able to recover the true values (Figure \ref{fig:ebola_synth_posts}) for all parameters, or identifiable functions of parameters in the case of the case detection rate. We essentially recover the priors for the parameters to which we assigned informative priors. 

Although the latent epidemic process in not identifiable in this model, we recover, for the most part, the mean incidence we would have expected to observe if the true incidence were known (Figure \ref{fig:ebola_synth_diags}). The partial posterior predictive distributions nicely capture the observed incidence curves, suggesting that the emission distribution adequately described the sampling process for the observed incidence, conditional on the latent incidence being distributed according to its posterior. While the full posterior predictive distributions for Guinea and Sierra Leone suggest that the model is an adequate data generating mechanism for the outbreaks in those countries. The full posterior predictive distribution for Liberia predicts slower dynamics than were actually observed, which is reflective of an estimated effective reproductive number for Liberia that was somewhat lower than the true value. 

The  and posterior predictive

The temporal full and partial predictive distributions for Guinea and Sierra Leone concord with the 

Finally,  Furthermore, excess variability in the LNA residuals at the beginning and end of the outbreak suggest that the LNA approximation might not adequately account for the uncertainty in the epidemic process during those times (see Figure (((())))). This, too, is expected since we know that there were few disease state transitions during those times. We present additional results and diagnostics in Section \ref{subsec:ebola_synth_supplement}. 

\begin{sidewaysfigure}[htbp]
	\begin{fullpage}
		\centering
		\includegraphics[width=\linewidth]{figures/ebola_synth_posts}
		\caption[Posterior distributions of stratified SEIR model parameters for a simulated Ebola outbreak in three countries.]{Posterior distributions of .}
...
\begin{figure}[htbp]
	\begin{fullpage}
		\centering
		\includegraphics[width=\linewidth]{figures/ebola_synth_pacfs}
		\caption[Posterior distributions of stratified SEIR model parameters for a simulated Ebola outbreak in three countries.]{Posterior distributions of .}
