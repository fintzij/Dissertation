...
\end{align}
where $ \beta_A $ is the per--contact rate of infection in country A, and the $ \alpha $ parameters are the rates of real or virtual migration of infected individuals from countries B and C. We note that the models are only approximately equivalent to the extent that the infectious period durations in each country are similar and that the contribution to the local force of infection from other countries is relatively minimal. The upside of this tradeoff is that the quantities, $ \alpha_{BA}I_B $ and $ \alpha_{CA}I_C$, are still interpretable as the effective number of infected individuals from countries B and C with whom susceptibles in country A come into contact. Moreover, the approximate model is more tractable from a computational perspective since we have fewer ODEs to solve in the LNA approximation, and fewer boundary conditions on the LNA state space. Arguably, the approximate model is also more appropriate since the LNA approximation for the numbers of migration events is tenuous due to low migration rates. The full model with migration was fit as a supplementary analysis and yielded equivalent results (see Section \ref{subsec:ebola_synth_fullmod_res}).  
