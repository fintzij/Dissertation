...
Each MCMC iteration will consist of a number of ElliptSS updates, typically one but possibly 2--3 for complex models, followed by a set of parameter updates. We will generally use either a global adaptive random walk Metropolis algorithm (Algorithm 4 in \cite{andrieu2008tutorial}), or a variant of the automated factor slice sampler of \cite{tibbits2014automated}, possibly in conjunction with a hit--and--run slice sampler to preserve ergodicity when updating in only some slice directions. Details of these algorithms are presented in Section \ref{subsubsec:slice_sampling}. 
