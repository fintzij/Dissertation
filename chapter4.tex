...
\subsubsection{Reparameterization the CLE in terms of comparment counts}
...
\end{equation}
Now, $ \bX $ is coupled to $ \bN $ via $ \bX(t) = \bx_0 + \bA^T\bN(t) $. For the SIR model, 
\begin{align}
\left (\begin{array}{c}
S(t) \\
I(t) \\
...
\rmd\bNtil &= \diag\left (\exp(-\bNtil(t) - 0.5\exp(-2\bNtil)\right )\blambda\left (\exp(\bNtil)-\bs{1}\right )\dt + \nonumber\\
& \hspace{0.5in} \diag\left (\exp(-\bNtil)\right )\bLambda\left (\exp(\bNtil)-1\right )^{1/2}\rmd\bW_t .
\end{align}

\subsection{Linear Noise Approximation --- SIR Model}
