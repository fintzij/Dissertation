...
\begin{table}
	\caption[Transmission rates for the stratified SEIR model for Ebola in West Africa.]{Rates of transitions between compartments for the full stratified SEIR model with cross--country transmission for the spread of Ebola in Guinea, Liberia, and Sierra Leone. The model is diagrammed in Figure \ref{fig:stratified_seir_full_diag}. Subscripts in the rates denote model compartments, superscripts denote countries. Subscripts for parameters denote countries. The rate at which susceptibles in Guine become infected is time varying with one changepoint at 33 weeks.}
	\label{tab:ebola_synth_rates_full}
	\centering\small
	\begin{tabular}{llll}
		\hline
		From & To & Country & Rate \\
		\hline
		Susceptible & Exposed & Guinea & $ \lambda_{SE}^G(t) = (\beta_G^{(1)} \ind{t<33} + \beta_G^{(2)}\ind{t\geq33})S_GI_G $ \\
		Susceptible & Exposed & Liberia & $ \lambda_{SE}^L = \beta_LS_LI_L $\\
		Susceptible & Exposed & Sierra Leone & $ \lambda_{SE}^S = \beta_SS_SI_S $ \\
		Exposed & Infected & Guinea & $ \lambda_{EI}^G = \omega_GE_G $\\
		Exposed & Infected & Liberia & $ \lambda_{EI}^L = \omega_LE_L $ \\
		...
		Infected (A) & Infected (B) & Countries A,B & $ \lambda_I ^{AB} = \alpha_{AB}I_A$\\
		\hline
	\end{tabular}
\end{table}
...
\subsection{Supplementary Analysis of the Simulated Ebola Outbreak with Modeling of Migration}
