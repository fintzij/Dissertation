...
When the scale of an outbreak is small relative to the population size, it may be unreasonable to assume that the entire population mixes homogeneously and participates in propagating the epidemic. A an alternative abstraction of the outbreak setting involves splitting the population into two (or more) sub--populations, one that participates in the outbreak and another that is separated from the outbreak. For example, in the case of the SIR model, we might have two susceptible compartments, $ S^R $ and $ S^E $, where $ S^R $ is a susceptible subpopulation that is effectively removed since they experience no infectious contact, and $ S^E $ is a susceptible population that may become exposed. In this model, the \textit{effective population size} is $ N_{eff} = S^E + I + R $. The model is otherwise constructed in the same way as the SIR model, except with $ S^E $ replacing $ S $. 

The effective population size and the mean case detection probability are weakly identifiable parameters, and we require prior information about their scales in order to fully disentangle their effects. In contrast estimate the mean case reporting rate, $ \rho\times N_{eff} $, is directly informed by the data. To see why this is so, note that $ \rho $ and $ N_{eff} $ enter into the complete data likelihood, (\ref{eqn:lna_ncp}), through the priors and through the product of emission probabilities in the observed data likelihood. These emission probabilities could be, for example, negative binomial distributions as in (\ref{eqn:incidence_emitprob}). The key point to note is that the mean of the emission distribution is of the form, $ \rho\Delta N_{SI}(t_\ell) $. Recalling that the LNA (and the MJP on which it is based) is a density dependent process \cite{komorowski2009,wilkinson2011stochastic} scaled by the size of the system, here $ N_{eff} $, we should actually understand the mean observed incidence to be the mean case detection probability multiplied by a change in concentrations and the effective population size, $ \rho\left (N^\prime_{S^EI}(t_{\ell}) - N^\prime_{S^EI}(t_{\ell-1})\right )N_{eff} $, where $ \bN^\prime = \bN/N_{eff} $ is the equivalent representation of the LNA in terms of compartment concentrations. Note that, although $ \rho $ and $ N_{eff} $ might seem to completely unidentifiable in light of this discussion, there are several points to support the assertion that they are weakly identifiable. First, certain stochastic aspects of the outbreak, such as the probability of a major outbreak and persistence of transmission, depend on the population size. Furthermore, the scale of the observed incidence and the observed outbreak duration are informative about the minimal effective population size. Therefore, the fact that we observe part of the outbreak is itself informative. Finally, a rough estimate the true population size is typically available, providing an upper bound on the effective population size.
...
We conclude by pointing out the important implication that the latent epidemic process is only weakly identifiable in models where the effective population size is not known. Uncertainty in the latent posterior reflects the extent that we have (or lack) prior knowledge of the effective population size or reporting rate. On the other hand, the posterior predictive distributions are reliable since they depend on the mean case reporting rate. Thus, the standard tools for model validation and prediction are still available to us.
...
\begin{sidewaystable}[htbp]
	...
	\begin{tabular}{lcllr}
		...
		$ R_{eff,L} $ & 2.1 & Effective reproduction \# & $ R_{eff,L}\sim $ LogNormal(log(0.5), 1.1) & $ \implies R_{eff,L} = 1.50 (1.06, 5.32)$ \\
		$ R_{eff,S} $ & 1.6 & Effective reproduction \# & $ R_{eff,S}\sim $ LogNormal(log(0.5), 1.1) & $ \implies R_{eff,S} = 1.50 (1.06, 5.32)$ \\
